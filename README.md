...
3. **运行采集器**：打开 `collector` 文件夹，在**地址栏**输入 `cmd` 回车（会直接弹出定位到该目录的命令行），然后：
   ...
   ```
4. **停止**：按 **Ctrl + C** 一次（或在看板点「⏹ 停止采集」）。
5. **结果文件**：`collector\output\<时间戳>\` 文件夹——内含 jsonl 原始数据与自动生成的自包含 HTML 报告；看板历史报告页也可直接打开。

- 默认每 **1 秒**落盘一个采样点；jsonl 首行为 meta 事件（核数/进程名/设备信息），事件行不参与统计。
...
- [ ] PSS 内存有数值，且随游戏内操作波动
- [ ] Ctrl+C 停止后生成 JSONL 文件与自包含 HTML 报告，内容可读、时间戳连续
- [ ] 切后台（小游戏退到微信主页）后 FPS 归零/无输出，切回恢复 —— 验证进程识别正确

> 换新机型时：按 `devices.md`「换机验收步骤」执行（--show-foreground → 30 秒试采 → 排查顺序）。

## 文档索引

| 文档 | 状态 | 说明 |
|---|---|---|
| `README.md` | ✅ 现行 | 本文件：获取/上手/参数/FAQ |
| `AGENTS.md` | ✅ 现行 | **AI 协作者必读**：协作规约、文件域分工、口径速查、命令与自检清单 |
| `CHANGELOG.md` | ✅ 现行 | 变更日志 + 当前状态快照（接手先读最新 1~2 条） |
| `使用教程-保姆级.md` | ✅ 现行（教程向） | 面向零基础同事的分步图文教程，与本 README 互补 |
| `指标说明.md` | ✅ 现行 V0.4 | 指标口径权威文档：Jank 节奏校准、统计口径、数据健全性自检 |
| `架构设计.md` | ✅ 现行 V1.0 | 模块架构、数据流、关键设计决策、里程碑 |
